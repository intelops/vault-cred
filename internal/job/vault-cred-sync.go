...
func (v *VaultCredSync) Run() {
	...
	for key, secretValue := range secretValues {
		if strings.HasPrefix(key, serviceCredSecretKeyPrefix) {
			err = storeServiceCredential(ctx, vc, key, secretValue)
			if err != nil {
				v.log.Errorf("%s", err)
				continue
			}
		} else if strings.HasPrefix(key, certSecretKeyPrefix) {
			err = storeCertData(ctx, vc, key, secretValue)
			if err != nil {
				v.log.Errorf("%s", err)
				continue
			}
...
func storeServiceCredential(ctx context.Context, vc *client.VaultClient, secretIdentifier, secretData string) error {
	...
	secretPath := api.PrepareCredentialSecretPath(certSecretKeyPrefix, serviceCredData.EntityName, serviceCredData.UserName)
...
func storeCertData(ctx context.Context, vc *client.VaultClient, secretIdentifier, secretData string) error {
	...
	secretPath := api.PrepareCredentialSecretPath(certSecretKeyPrefix, certData.EntityName, certData.CertIndentifier)
	err = vc.PutCredential(ctx, api.CredentialMountPath(), secretPath, cred)
	if err != nil {
		return errors.WithMessagef(err, "failed to write %s secret data to vault", secretIdentifier)
	}
