...
func NewVaultClient(log logging.Logger, cfg *Config) (*VaultCredServ, error) {
	log.Infof("connecting to VaultClient %s", cfg.Address)
	...
	vaultClient := vaultcredpb.NewVaultCredClient(conn)
...
func parseAgentConnectionConfig(vaultAddress string) (agentHost, agentPort string, tlsEnabled bool, err error) {
	var parsedURL *url.URL
	parsedURL, err = url.Parse(vaultAddress)
	if err != nil {
		return
	}
...
// func (a *VaultCredServ) Ping(ctx context.Context, request *vaultcredpb.Pin) (*vaultcredpb.PingResponse, error) {
// 	a.log.Infof("Ping request received")
// 	return &vaultcredpb.PingResponse{Status: vaultcredpb.StatusCode_OK}, nil
// }
